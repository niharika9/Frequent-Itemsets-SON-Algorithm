...
object Niharika_Gajam_SON_large{
...
//    val input_file = spark_context.textFile("/Users/nihagajam/Desktop/INF 553/HW3/inf553_assignment3/Data/yelp_reviews_large.txt",4).cache() //args(0)

   val input_file = spark_context.textFile(args(0),4).cache()

    val support_threshold = args(1).toInt

    val outputfile  = new BufferedWriter(new FileWriter(args(2))) //args(2)
    ...
    val chunk_threshold = support_threshold/(baskets_items.getNumPartitions)
    ...
    loop.breakable {
      ...
      while(true) {
        ...
        val op = output_phase2.filter(x => x.size == set_sz.toInt).map(x => x.toString).sortBy(x => x, true, numPartitions = 1).collect()
